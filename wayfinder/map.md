...
_Empty — the fog is fully cleared._ Every in-scope decision is now in **Decisions so far**; the only remaining ticket is the AFK assembly of those decisions into the spec ([Assemble the implementation-ready spec](tickets/07-task-assemble-spec.md)), which invents nothing new.
